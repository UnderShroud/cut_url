...
function get_users_count() {
  return db_query("SELECT COUNT('id') FROM `users`;");
}

function get_links_count() {
  return db_query("SELECT COUNT('id') FROM `links`;");
}

function get_views_count() {
  return db_query("SELECT SUM(`views`) FROM `links`;");
}
...
function login($auth_data) {
  if (empty($auth_data) || !isset($auth_data['login']) || empty($auth_data['login']) || !isset($auth_data['pass']) ) 
  {
  ...
  $user = get_user_info($auth_data['login']);
  if (empty($user)) {
    $_SESSION['error'] = "Пользователь ".$user." не существует";
    header('Location: register.php');
    die;
  }
